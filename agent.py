...
def log_help_request(customer_id: str, question: str) -> None:
    ...
    with get_db_connection() as conn:
        ...
        conn.commit()
    logger.info(f"📞 SUPERVISOR ALERT: Need help with: '{question}' from {customer_id}")
...
async def send_heartbeat(customer_id: str) -> None:
    ...
    logger.info(f"📊 Started activity tracking for {customer_id}, session: {session_id}")
...
async def check_supervisor_answers(session: AgentSession, customer_id: str) -> None:
    ...
    logger.info("🔄 Starting supervisor answer monitoring...")
    ...
    while True:
        try:
            ...
            for request in answered_requests:
                ...
                logger.info(f"📢 New supervisor answer for request #{request_id}! Question: {question} | Answer: {answer}")
                ...
                try:
                    ...
                    msg = str(speak_err)
                    logger.error(f"❌ Failed to deliver answer via voice: {msg}")
                    ...
                    if 'timed out' in msg.lower() or 'disconnect' in msg.lower() or 'cancelled' in msg.lower():
                        logger.warning("🛑 Stopping supervisor monitoring due to inactive session")
                        return
                    # Otherwise continue loop
                    continue
                logger.info(f"✅ Delivered supervisor answer to customer via voice!")
...
async def search_knowledge_base(question: str) -> str:
    ...
    if answer:
        logger.info(f"✅ Found answer in KB: {answer[:50]}...")
        return answer
    else:
        logger.warning(f"❌ No answer found, escalating to supervisor")
...
async def request_supervisor_help(question: str, customer_name: str, customer_phone: str) -> str:
    ...
    logger.info(f"👤 Customer: {customer_name}, Phone: {customer_phone}")
    
    # Log to database with the active session's customer identifier - just the question text
    log_help_request(CURRENT_CUSTOMER_ID or customer_name or "UnknownCustomer", question)
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info(f"👤 Customer connected: {customer_id}")
    ...
    logger.info("🤖 Voice assistant session created with Gemini Multimodal Live API!")
    
    # Start background tasks with customer_id
    heartbeat_task = asyncio.create_task(send_heartbeat(customer_id))
    logger.info(f"💚 Heartbeat started for {customer_id} - Dashboard will show activity!")
    supervisor_monitor_task = asyncio.create_task(check_supervisor_answers(session, customer_id))
    logger.info(f"✅ Supervisor answer monitoring started for {customer_id}!")
